...
public class WebServer1 {
    ...
    private void handleCalcularRequest(HttpExchange exchange) throws IOException {
        ...
        byte[] requestBytes = exchange.getRequestBody().readAllBytes();
        byte[] responseBytes = comprobarPrimo(requestBytes);
    ...
    private byte[] comprobarPrimo(byte[] requestBytes) {
        String bodyString = new String(requestBytes);
        String[] stringNumbers = bodyString.split(",");

        String result = isPrime(Integer.parseInt(stringNumbers[0])) ? "Es primo" : "No es primo";
        System.out.println("Server resultado: " + result);
        return String.format("El numero %s %s\n", stringNumbers[0], result).getBytes();
    }

    public static boolean isPrime(int input_number) {
        if (input_number <= 1) {
            return false;
        }
